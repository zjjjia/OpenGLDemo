...
public class GLMovieRecorder {
    ...
    private TransitionVideoRender mRender;
    ...
    public void setDataSource(TransitionVideoRender render) {
        mRender = render;
    }
    ...
    public void startRecord(final OnRecordListener listener) {
        ...
        if (mRender == null) {
            throw new RuntimeException("please setDateSource first");
        }
    ...
    private void startRecordImpl(OnRecordListener listener) throws IOException {
        prepareEncoder();

        mInputSurface.makeCurrent();

        mRender.setViewport(mWidth, mHeight);
        mRender.setRenderToRecorder(true);
        ...
        if (!TextUtils.isEmpty(mAudioPath)) {
            ...
            audioRecordThread = new AudioRecordThread(mContext, mAudioPath, mMuxer, mCyclicBarrier, totalDuration);
        ...
        try {
            while (tempTime <= (totalDuration + ONE_BILLION / 30)) {
                drainEncoder(false);
                mRender.drawFrame(tempTime);
                long presentationTimeNsec = computePresentationTimeNsec(frameCount);
                ...
                if (listener != null) {
                    listener.onRecordProgress(tempTime, totalDuration);
                }
            }
            drainEncoder(true);
        } finally {
            releaseEncoder();
            mRender.setRenderToRecorder(false);
    ...
    public interface OnRecordListener {
        ...
        void onRecordProgress(long recordedDuration, long totalDuration);
    }
}
